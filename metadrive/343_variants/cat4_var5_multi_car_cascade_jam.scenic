...
# Variant: c4_v5_cascade_jam
# Modified vehicles: car1->delayedLeftTurnCut(L_centerlines), car3->hardBrakeInFront(), car4->steerAcrossLaneLeft(), car5->suddenSprintStraight(), car6->fastApproachFromBehind(), car9->stagedLaneChangeAttack(), car11->intersectionBlocker()

ego = new Car at -124 @ -16.6, with behavior EgoBehavior(), with sensors {
        "front_rgb": RGBSensor(offset=(0.3, 0, 1.7), width=1280, height=720)
    }
car1 = new Car at -128.5 @ 16, with behavior delayedLeftTurnCut(L_centerlines)
car2 = new Car at -128.5 @ 27, with behavior leftTurnBehavior(20, L_centerlines)
car3 = new Car at -132 @ 16, with behavior hardBrakeInFront()
...
car11 = new Car at -132 @ 41, with behavior intersectionBlocker()
