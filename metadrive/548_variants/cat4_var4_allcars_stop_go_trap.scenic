...
behavior HesitateThenAccelerate():
    do FollowLaneBehavior(0) for 5 seconds
    do FollowLaneBehavior(12) for 2 seconds
    do FollowLaneBehavior(65) for 13 seconds
behavior LateSwerveRight():
    try:
        do FollowLaneBehavior(10) for 5 seconds
        do FollowLaneBehavior(28) for 15 seconds
    interrupt when 7 * STEPS_PER_SEC < simulation().currentTime and simulation().currentTime < 10 * STEPS_PER_SEC:
...
behavior SurgeThenStop():
    try:
        do FollowLaneBehavior(45) for 5 seconds
        do FollowLaneBehavior(0) for 15 seconds
    interrupt when 5 * STEPS_PER_SEC < simulation().currentTime and simulation().currentTime < 9 * STEPS_PER_SEC:
        take SetBrakeAction(1.0)
# Five modified vehicles: stop-go trap.
ego = new Car at -262.11 @ -425.37, with behavior EgoBehavior(),
    with sensors {
        "front_rgb": RGBSensor(offset=(0.3, 0, 1.7), width=1280, height=720)
    }
car2 = new Car at -197.14 @ -428.11, with behavior SurgeThenStop()
car3 = new Car at -191.35 @ -428.69, with behavior PanicBrake()
car4 = new Car at -185.67 @ -431.40, with behavior RollingBlock()
car5 = new Car at -195.21 @ -432.27, with behavior HesitateThenAccelerate()
car6 = new Car at -204.83 @ -431.29, with behavior LateSwerveRight()
