...
behavior ZigZagPush():
    ...
    interrupt when 7 * STEPS_PER_SEC < simulation().currentTime and simulation().currentTime < 8 * STEPS_PER_SEC:
        take SetSteerAction(0.45)
# Two modified vehicles: zig-zag plus aggressive surge.
...
car5 = new Car at -195.21 @ -432.27, with behavior ZigZagPush()
car6 = new Car at -204.83 @ -431.29, with behavior AggressiveLaunch()
