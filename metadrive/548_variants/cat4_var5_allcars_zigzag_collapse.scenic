...
behavior AggressiveLaunch():
    do FollowLaneBehavior(0) for 1.0 seconds
    do FollowLaneBehavior(85) for 20 seconds
...
behavior ZigZagPush():
    try:
        do FollowLaneBehavior(22) for 20 seconds
    interrupt when 5 * STEPS_PER_SEC < simulation().currentTime and simulation().currentTime < 6 * STEPS_PER_SEC:
        take SetSteerAction(0.45)
    interrupt when 6 * STEPS_PER_SEC < simulation().currentTime and simulation().currentTime < 7 * STEPS_PER_SEC:
        take SetSteerAction(-0.45)
    interrupt when 7 * STEPS_PER_SEC < simulation().currentTime and simulation().currentTime < 8 * STEPS_PER_SEC:
        take SetSteerAction(0.45)
# Five modified vehicles: unstable zig-zag collapse.
ego = new Car at -262.11 @ -425.37, with behavior EgoBehavior(),
    with sensors {
        "front_rgb": RGBSensor(offset=(0.3, 0, 1.7), width=1280, height=720)
    }
car2 = new Car at -197.14 @ -428.11, with behavior ZigZagPush()
car3 = new Car at -191.35 @ -428.69, with behavior BrakeThenCutLeft()
car4 = new Car at -185.67 @ -431.40, with behavior BrakeThenCutRight()
car5 = new Car at -195.21 @ -432.27, with behavior AggressiveLaunch()
car6 = new Car at -204.83 @ -431.29, with behavior CreepAndDriftLeft()
